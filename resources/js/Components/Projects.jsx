...
export default function Projects() {
  ...
  const fetchContentDetails = async (pageId) => {
    try {
      const response = await fetch(`https://ksbuilder.wolfczartech.net/${pageId}`, {
  ...
  return (
    <div >
      <div className=" relative my-[20px] flex justify-center items-center h-[40px] px-[5%] " >
        <h1 className=" text-[30px] text-[#333333] font-semibold  " >Projects</h1>
        {/* <button className="" >Edit</button> */}
      </div>
      <div className="breadcrumb mb-[40px]" onClick={() => { setContentSectionTarget('1 projects'), setSectionId(findObject('1 projects')?.section_content?.[0]?.id) }}>
        <a href="#" className=" font-semibold " >KSK BUILDER</a>
        <span className=" font-semibold " onClick={() => { }} >Projects</span>
