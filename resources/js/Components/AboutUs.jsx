...
export default function AboutUs() {
  ...
  function SaveForm(data) {
    console.log(data?.imageType, 'data ::')
    const actions = {
      '1 AboutUs': saveSection
    }
    actions[contentSectionTarget]?.(data, findObject(rendingData(data?.imageType))?.section_content?.[0]?.id);
  }
  ...
  return (
    <div >
      ...
      <div className="breadcrumb mb-[40px]" onClick={() => { setContentSectionTarget('1 AboutUs'), setSectionId(findObject('1 AboutUs')?.section_content?.[0]?.id) }}>
        <a href="#" className=" font-semibold " >KSK BUILDER</a>
        <span className=" font-semibold " onClick={() => { }} >ABOUT US</span>
        {routeUrl() &&
